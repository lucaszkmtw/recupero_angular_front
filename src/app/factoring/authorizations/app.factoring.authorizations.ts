angular.module('app.factoring.authorizations', ['app.factoring'])
    .config(['$stateProvider', $stateProvider => {
        $stateProvider
            .state('app.factoring.authorizations',
            {
                url: '/authorizations',
                controller: 'FactoringAuthorizationsController',
                templateUrl: 'app/factoring/authorizations/list.html',
                resolve: loadSequence('jqueryui', 'jqGrid', 'toastr'),
                ncyBreadcrumb: {
                    label: 'lns.authorizations'
                }
            })
            .state('app.factoring.authorization',
            {
                url: '/authorizations/{operationId}',
                controller: 'FactoringAuthorizationController',
                templateUrl: 'app/factoring/authorizations/view.html',
                ...
                ncyBreadcrumb: {
                    parent: 'app.factoring.documentapprovals',
                    label: 'lns.authorization'
                }
            });
    }])
    .controller('FactoringAuthorizationsController', ['$scope', '$translate', '$state', 'Restangular', ($scope, $translate, $state, Restangular) => {
        ...
        $scope.view = (id) => {
            $state.go('app.factoring.auhtorization', { operationId: id });
        };
    ...
    .controller('FactoringAuthorizationController', ['$scope', '$translate', '$stateParams', '$state', 'Restangular', 'toastr', ($scope: IOperationsAuhtorizationScope, $translate, $stateParams, $state, Restangular, toastr) => {

        var id = $stateParams.operationId;
        $scope.options = { edit: false };

        $scope.assign = (roleId) => {
            Restangular.service('system/workflows/workflowinstances/' + $scope.operation.approvalWorkflowInstance.guid + '/assign').post({ roleId: roleId }).then(() => { load(); });
        };

        function load() {
            if (id) {
                Restangular.one('factoring').one('operations', id).get().then(result => {
    ...
    .directive('factoringAuthorizationsGrid', ['$state', '$log', '$compile', '$filter', 'Restangular', 'toastr', 'authManager', ($state, $log, $compile, $filter, Restangular, toastr, authManager) => {
        return {
            restrict: 'A',
            ...
            link(scope: any, element) {
                var tabsElement = '<div><uib-tabset>'
                    ...
                    + '</uib-tabset></div>';
                var gridElementName = 'factoringAuthorizationsGrid';
                ...
                function dataFormatter(cellvalue: any, options: any, rowObject: any) {
                    var createDate = $filter('amDateTime')(rowObject.date);
                    var amount = $filter('currency')(rowObject.amount);
                    return '<small>Solicitante</small> <a data-ui-sref="app.system.person({ personId: ' + rowObject.personId + ' })" title="Ver ficha">' + rowObject.personName + '</a><br><small>Creada el ' + createDate + '</small><br><small>Monto solicitado ' + amount + '</small></td>';
                }
                ...
                function documentActionsFormatter(cellvalue: any, options: any, rowObject: any) {
                    var template = '<a href="#" data-ui-sref="app.factoring.operation({ operationId: ' + rowObject.id + '})" class="btn btn-white btn-xs m-l-md"><i class="fa fa-folder"></i> Ver </a>';
                ...
                var colModel: Array<any> = [
                    ...
                    { name: 'operationData', index: 'operationData', search: false, formatter: dataFormatter },
                ...
                function loadData() {
                    var url = '/app/factoring/data/authorizations.json';
