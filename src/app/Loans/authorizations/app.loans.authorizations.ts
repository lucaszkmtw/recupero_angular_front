angular.module('app.loans.authorizations', ['app.loans'])
    .config(['$stateProvider', $stateProvider => {
        $stateProvider
            .state('app.loans.authorizations',
            {
                url: '/authorizations',
                controller: 'LoansAuthorizationsController',
                templateUrl: 'app/loans/authorizations/list.html',
                resolve: loadSequence('jqueryui', 'jqGrid', 'toastr'),
                ncyBreadcrumb: {
                    label: 'lns.authorizations'
                }
            })
            .state('app.loans.authorization',
            {
                url: '/authorizations/{loanId}',
                controller: 'LoansAuthorizationController',
                templateUrl: 'app/loans/authorizations/edit.html',
                resolve: loadSequence('ngCkeditor',
                    ...
                    'toastr'),
                ncyBreadcrumb: {
                    parent: 'app.loans.documentapprovals',
                    label: 'lns.authorization'
                }
            });
    }
    ])
    .controller('LoansAuthorizationsController', ['$scope', '$translate', '$state', 'Restangular', ($scope, $translate, $state, Restangular) => {
        $scope.params = {
            filter: null,
            view: 0,
            selectedItems: []
    ...
    .controller('LoansAuthorizationController', ['$scope', '$translate', '$stateParams', '$state', 'Restangular', 'toastr', ($scope: ILoansAuhtorizationScope, $translate, $stateParams, $state, Restangular, toastr) => {
        var id = $stateParams.loanId;
        $scope.options = { edit: false };

        $scope.assign = (roleId) => {
            Restangular.service('system/workflows/workflowinstances/' + $scope.loan.approvalWorkflowInstance.guid + '/assign').post({ roleId: roleId }).then(() => { load(); });
        };

        $scope.approve = () => {
            Restangular.service('system/workflows/workflowinstances/' + $scope.loan.approvalWorkflowInstance.guid + '/approve').post({}).then(() => {
        ...
        $scope.reject = () => {
            Restangular.service('system/workflows/workflowinstances/' + $scope.loan.approvalWorkflowInstance.guid + '/reject').post({}).then(() => {
                toastr.success('Autorizaciones', 'La operación se realizó con éxito.');
                load();
            }, () => {
                toastr.error('Autorizaciones', 'Se produjo un error en la operación.');
        ...
        function load() {
            if (id) {
                Restangular.one('loans').one('loans', id).get().then(result => {
                    $scope.loan = result;
                });
            } else {
                $scope.loan = {};
    ...
    .directive('loansLoansAuthorizationsGrid', ['$state', '$log', '$compile', '$filter', 'Restangular', 'toastr', 'authManager', ($state, $log, $compile, $filter, Restangular, toastr, authManager) => {
        return {
            restrict: 'A',
            ...
            link(scope: any, element) {
                ...
                function documentActionsFormatter(cellvalue: any, options: any, rowObject: any) {
                    var template = '<a href="#" data-ui-sref="app.loans.loan({ loanId: ' + rowObject.id + '})" class="btn btn-white btn-xs m-l-md"><i class="fa fa-folder"></i> Ver </a>';
                ...
                function loadData() {
                    var url = '/api/loans/loans/authorizations.json';
